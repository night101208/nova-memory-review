...
| 지적 | 처리 |
|---|---|
| 저장 중 메시지 전송 경합 (`sendDraft`에 guard 없음) | **고침.** `sendDraft()` 맨 앞에 guard, `ChatView`의 입력란·전송 버튼도 잠급니다. ⚠️ `draft = ""`가 guard보다 **앞에** 있어서 그대로 막았으면 입력한 글까지 날아갔습니다 — 순서를 바꿔 입력은 남기고 상태줄로 이유를 알립니다 |
| 1·2·3·5번 | 검사 의견대로 현행 유지 |
